...
var parseNumberFromServer = function(inputObjectsBranche){





    return _.mapValues(inputObjectsBranche, function(value, key){
...
scope.$on('removeTable', function(event, data){


              _.remove(scope.treeData);
              _.remove(scope.colDefs);
  ...
  .filter('searchFor', function() {
		return function(arr, filterString, expandingProperty, colDefinitions) {
          ...
          if (_.isUndefined(arr)){
            ...
            //console.log('Ошибка разработчика, необходимо доделать фильтрацию.');




          }else if(!filterString || filterString.length < 3){
			   ...
			   for (var i = 0; i < arr.length; i++) {
		              ...
		              if (item.visible) {
                        ...
                        _.assign(item.branch, parseNumberFromServer(item.branch));
