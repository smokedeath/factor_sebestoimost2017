...
@Component({
    moduleId: module.id,
    selector: 'usl-rashodstavok-component',
    templateUrl: 'usl-rashodstavok-component.html'
})

export class UslRashodStavokComponent implements OnInit{      
    ...
    getTabelData(){
        ...
        // Таблица      
        this.service.getRashodstavokUsl(param, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
