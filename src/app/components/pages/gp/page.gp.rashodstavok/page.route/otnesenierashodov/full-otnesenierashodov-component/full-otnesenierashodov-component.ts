...
@Component({
    moduleId: module.id,
    selector: 'full-otnesenierashodov-component',
    templateUrl: 'full-otnesenierashodov-component.html'
})

export class FullOtnesenieRashodovComponent implements OnInit{      
    ...
    getTabelData(){
        ...
        // Таблица      
        this.service.getOtnesenierashodovFull(param, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
