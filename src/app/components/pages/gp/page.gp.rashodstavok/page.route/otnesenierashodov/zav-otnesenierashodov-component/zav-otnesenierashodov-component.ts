...
@Component({
    moduleId: module.id,
    selector: 'zav-otnesenierashodov-component',
    templateUrl: 'zav-otnesenierashodov-component.html'
})

export class ZavOtnesenieRashodovComponent implements OnInit{      
    ...
    getCube(){
        ...
        let data = {
            ...
            cubeConst: 'dependentExpensesCube',
            filter: s
        }
                    ...
                    .subscribe(
                        data => {
                            if (data.status==200){
                                ...
                                let sizeName = data[this.service.cubExplPokazZavParams.sizeName];
                                ...
                                this.noFixetColumns = [];
                                let tableX = data[this.service.cubExplPokazZavParams.tableX];
                                for (let i=0; i<tableX.length; i++){
                                    this.noFixetColumns.push({field: tableX[i].id, header: tableX[i].name});
                                }
                                this.initTableColumns();

                                let tableY = data[this.service.cubExplPokazZavParams.tableY];
                                this.tableY0 = [];
                                this.tableYChild = {};
    ...
    inputCoardinate(data){
        for (let i=0; i<data.length; i++){
            for (let b=0; b<this.coardinat.length; b++){
                if (this.coardinat[b][this.service.cubExplPokazZavParams.tableY]==data[i].data.id){
                        ...
                        if (this.arrItemSize[q].sizeParent==this.coardinat[b].measure){
                            for (let w=0; w<this.arrItemSize[q].size.length; w++){
                                if (this.arrItemSize[q].size[w].id==this.arrItemSize[q].sizeModel){
                                    ...
                                    data[i].data[this.coardinat[b][this.service.cubExplPokazZavParams.tableX]] = rounded;
                                }
                            }
                        }
                    }
    ...
    ngOnInit(){
        ...
        this.service.getCubeDimData({session: this.user.session, cubeConst: 'dependentExpensesCube', dimNameConst: 'freightCarrierDo' }, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
        ...
        // поставщики
        this.service.getCubeDimData({session: this.user.session, cubeConst: 'dependentExpensesCube', dimNameId: 'dp_q' }, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
        ...
        //статусы
        this.service.getCubeDimData({session: this.user.session, cubeConst: 'dependentExpensesCube', dimNameId: 'dp_s' }, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
