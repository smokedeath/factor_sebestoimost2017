...
@Component({
    moduleId: module.id,
    selector: 'usl-otnesenierashodov-component',
    templateUrl: 'usl-otnesenierashodov-component.html'
})

export class UslOtnesenieRashodovComponent implements OnInit{      
    ...
    getTabelData(){
        ...
        // Таблица      
        this.service.getOtnesenierashodovUsl(param, this.user.programmId, this.userSetings.langId)
                    .subscribe(
                        data => {
                            if (data.status==200){
                                data = data.json();
